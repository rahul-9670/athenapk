...
class PoissonEquation {
 public:
  ...
  // separate tiny physical-BC kernel per (block, face). In the deeply-refined collapse that
  // per-block dispatch dominates the GPU launch/sync latency (Grete 2026-07: ~7e5 GenericBC
  // launches in a 4-cycle trace). Here we apply the self-gravity phi BCs (zero-Dirichlet /
  // Neumann-outflow, uniform Cartesian) to the WHOLE MeshData in ONE par_for per face, gated
  // by a per-block physical-boundary flag. The per-face iteration space and the FixedFace /
  // outflow ghost formulas are taken verbatim from parthenon's GenericBC, so the result is
  // bit-identical to the per-block path. Any non-(zero|neumann) face -> delegate to the
  // original per-block path so correctness is never at risk.
  static parthenon::TaskStatus
  SetBoundary(std::shared_ptr<parthenon::MeshData<Real>> &md, bool coarse) {
    using namespace parthenon;
    auto pkg = md->GetBlockData(0)->GetBlockPointer()->packages.Get("self_gravity");
    // Per-face BC type: 0 = other/default (delegate), 1 = zero (FixedFace 0),
    // 2 = neumann, 3 = multipole (WS-5a).
    ...
    // Delegate whenever the packed path is disabled or any active face is not one of the
    // handled types (zero / neumann / multipole).
    //
    // Multipole (type 3): the *operator* the Krylov/MG solver applies must stay LINEAR, so
    // inside the solve the multipole faces are treated as HOMOGENEOUS zero-Dirichlet
    // (ghost = -mirror, exactly like type 1). The inhomogeneous exterior-expansion data is
    // instead lifted into the RHS in FillPoissonRHS (b -= 2*Phi_mp/dx_n^2 at boundary-
    // adjacent cells). The physical (inhomogeneous) ghost values are restored only AFTER
    // the solve, by the enrolled per-block MultipoleBC in AddBoundaryExchangeTasks, so
    // ApplyGravitySource sees the correct halo. (Applying 2*val-mirror inside the solve
    // makes the operator affine and diverges the preconditioned solve.)
    bool packable = use_packed;
    for (int f = 0; f < 6 && packable; ++f)
      if (face_active(f) && bctype[f] != 1 && bctype[f] != 2 && bctype[f] != 3)
        packable = false;
    ...
    // NumBlocks), so we map each pack block to its MeshBlock by GID (q.GetGIDHost) rather than
    // assuming a 1:1 md-block correspondence. Blocks not carrying phi are simply not in the
    // pack and (correctly) receive no BC -- the per-block path skips them too. This lets the
    // packed path cover the multigrid levels as well as the leaf grid.
    ...
    for (int pb = 0; pb < npb; ++pb) {
      ...
      // per-block ApplyBoundaryConditionsOnCoarseOrFine skips the rest, boundary_conditions
      // .cpp:46) -- touching an unallocated coarse buffer is illegal, so gate on it here.
    ...
    for (int f = 0; f < 6; ++f) {
      ...
      // Iteration space: the library's boundary domain (identical to GenericBC's par_for_bndry).
      ...
      parthenon::par_for(
          ...
          KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
            ...
            // type 1 (zero) and type 3 (multipole) both apply the HOMOGENEOUS Dirichlet
            // operator here: ghost = 2*0 - mirror(interior). For multipole the nonzero face
            // value is carried in the RHS (see FillPoissonRHS) so the operator stays linear.
            if (tt == 1 || tt == 3) {
              const int km = (dd == 2) ? offc - k : k;
              const int jm = (dd == 1) ? offc - j : j;
              const int im = (dd == 0) ? offc - i : i;
              q(b, te, var_t(), k, j, i) = -q(b, te, var_t(), km, jm, im);
            } else { // tt == 2: neumann / outflow: copy the interior boundary cell along DIR
