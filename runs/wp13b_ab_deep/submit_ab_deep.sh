...
#SBATCH --output=/beegfs/u/bbg6470/athenapk/runs/wp13b_ab_deep/%x_%j.out
set -o pipefail
#
...
# in one 1h45m 4-GPU slot. Internal -t 05:10:00 therefore passes the epoch with ~30 % margin while
...
# --- GPU state BEFORE we allocate anything -------------------------------------------------
# Leg 2492769 (this same leg) died at 38 s with 78.1 GiB on one of its four H100s. I first blamed
# "an occupied card at init" and resubmitted with --exclusive; probe job 2493185 FALSIFIED that:
# SLURM masks the GRES (SLURM_JOB_GPUS=2,5,6,7 while CUDA_VISIBLE_DEVICES=0,1,2,3, nvidia-smi -L
# counts 4 not 8), so wrap_mod.sh's local_rank % NGPU hands the four ranks four DISTINCT logical
# devices, and with GresEnforceBind=Yes no other job can be on our physical cards. Neither the
# rank-collision nor the shared-card story survives. --exclusive was therefore aimed at a cause
# that does not exist, and it was the ONLY reason the resubmit sat at Reason=Resources for ~9 h
# while g001 and g004 each had 5 idle H100s. Dropped.
# The actual cause is still unknown -- one rank really did ask for ~78 GiB -- because the run.log
# with the CUDA error was deleted when I cleaned $D for the resubmit. These two lines make the
# next failure readable instead of repeating that mistake; they run before mpirun and touch
# nothing the physics sees.
...
stdbuf -oL -eL mpirun -n 4 $MCA $WRAP $BIN -i $D/fhc_ens.in -t 05:10:00 \
