#!/bin/bash
#SBATCH --job-name=ct_coal_repro2
...
srun --mpi=pmix -n 8 $BIN -i repro2.in parthenon/mesh/do_coalesced_comms=false \
  -d off > off/log 2>&1
...
rm -rf on; mkdir -p on
srun --mpi=pmix -n 8 $BIN -i repro2.in parthenon/mesh/do_coalesced_comms=true \
  -d on > on/log 2>&1
