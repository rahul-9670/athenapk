...
OUT=confirm_ctfix; mkdir -p $OUT
echo "=== CT-fix confirm $(date) binary=$(md5sum $BIN|cut -c1-8) restart fc128b/out2.00002 t=1.100 -> tlim=1.10291 ===" >> $OUT/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN -r fc128b/parthenon.out2.00002.rhdf -t 05:45:00 \
  parthenon/mesh/nx1=128 parthenon/mesh/nx2=128 parthenon/mesh/nx3=128 \
  parthenon/mesh/numlevel=14 refinement/curr_max_level=4 parthenon/time/tlim=1.10291 parthenon/time/nlim=100000000 \
  ...
  parthenon/output1/dn=100 parthenon/output2/dn=100000 \
