...
COMMON="parthenon/mesh/nx1=128 parthenon/mesh/nx2=128 parthenon/mesh/nx3=128 \
  ...
  diffusion/integrator=rkl2 diffusion/rkl2_max_dt_ratio=400 \
  diffusion/rkl2_freeze_eta=true diffusion/eta_ohm_cap_code=0.1 diffusion/eta_hall_cap_code=0.05 \
  diffusion/ion_zeta=1.0e-16 diffusion/resistivity=none diffusion/hall=none"

for CD in ct glm; do
  OUT=diag_${CD}3; rm -rf $OUT; mkdir -p $OUT
