...
OUT=probe_glm; mkdir -p $OUT
RESTART=fc128c/parthenon.out2.00010.rhdf
echo "=== GLM-vs-CT test $(date) restart=$RESTART cap=10 first_order_flux_correct=true tlim=1.1050 ===" >> $OUT/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN -r $RESTART -t 01:20:00 \
  ...
  hydro/first_order_flux_correct=true hydro/divergence_control=glm \
