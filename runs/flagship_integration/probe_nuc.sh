...
OUT=probe_nuc; mkdir -p $OUT
RESTART=fc128c/parthenon.out2.00010.rhdf
echo "=== nucleation probe cap=10 $(date) restart=$RESTART per-cycle phdf tlim=1.10360 ===" >> $OUT/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN -r $RESTART -t 00:35:00 \
  parthenon/mesh/nx1=128 parthenon/mesh/nx2=128 parthenon/mesh/nx3=128 \
  parthenon/mesh/numlevel=14 refinement/curr_max_level=4 parthenon/time/tlim=1.10360 parthenon/time/nlim=100000000 \
