...
# the one non-ideal term lacking the cap Ohm/Hall have. New key diffusion/eta_ad_cap_code=3.0
...
N=$(cat fc128c_n 2>/dev/null||echo 0); N=$((N+1)); echo $N > fc128c_n
[ -f STOP_128c ] && { echo "STOP -> exit"; exit 0; }
NEWEST=$(ls -t fc128c/parthenon.out1.*.phdf 2>/dev/null|head -1)
if [ -n "$NEWEST" ]; then
  RM=$($PY - "$NEWEST" <<'PYEOF'
...
PYEOF
  ...
  [ "$($PY -c "print(1 if $RM*$RHO0>=$STOP_CGS else 0)")" = "1" ] && { echo "FIRST CORE rho=$RM -> STOP"; echo "first core $RM $(date)" > STOP_128c; exit 0; }
fi
# First slot restarts from the fc128b deep state; later slots chain from fc128c.
LATEST=$(ls -t fc128c/parthenon.out2.*.rhdf 2>/dev/null|head -1)
if [ -z "$LATEST" ]; then LATEST=$(ls -t fc128b/parthenon.out2.*.rhdf 2>/dev/null|head -1); fi
[ $N -ge 2 ] && [ -z "$(ls -t fc128c/parthenon.out2.*.rhdf 2>/dev/null|head -1)" ] && { echo "no fc128c restart -> STOP"; echo "no-restart" > STOP_128c; exit 0; }
...
mkdir -p fc128c
echo "=== fc128c slot $N $(date) 128^3 curr_max_level=4 eta_ad_cap=3.0 restart=$LATEST ===" >> fc128c/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN $RA -t 07:45:00 \
  ...
  diffusion/eta_ad_cap_code=3.0 \
  diffusion/ion_zeta=1.0e-16 -d fc128c >> fc128c/run.log 2>&1
echo "RUN_EXIT $? $(date)" >> fc128c/run.log
