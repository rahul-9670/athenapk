...
set -o pipefail
# FIX TEST: the t=1.10354 blowup = magnetic-wall velocity runaway -> gas pressure goes
# NEGATIVE (E<KE+ME) -> floored -> diverges, in a beta~1e-10 core-edge cell. Turn ON
# first_order_flux_correct (redo negative-P/rho cells with diffusive first-order LLF flux;
# untouched where P>0) + keep AD cap=10 for dt. Restart the SAME cyc1000 pre-blowup state
# and run PAST the event t=1.10354. Survives => fix works.
...
OUT=probe_boris; mkdir -p $OUT
RESTART=fc128c/parthenon.out2.00010.rhdf
echo "=== FOFC fix probe $(date) restart=$RESTART cap=10 first_order_flux_correct=true tlim=1.1040 ===" >> $OUT/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN -r $RESTART -t 01:20:00 \
  ...
  hydro/first_order_flux_correct=true hydro/boris_ca_max_code=300.0 \
  ...
  diffusion/eta_ad_cap_code=10.0 diffusion/ion_zeta=1.0e-16 -d $OUT >> $OUT/run.log 2>&1
