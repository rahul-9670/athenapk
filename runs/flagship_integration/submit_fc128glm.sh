...
# curr_max_level=4 hydro/divergence_control=glm bounds the current-sheet refinement (the deep-core block explosion) while Jeans
...
N=$(cat fc128glm_n 2>/dev/null||echo 0); N=$((N+1)); echo $N > fc128glm_n
[ -f STOP_128glm ] && { echo "STOP -> exit"; exit 0; }
...
PYEOF
  ...
  [ "$($PY -c "print(1 if $RM*$RHO0>=$STOP_CGS else 0)")" = "1" ] && { echo "FIRST CORE rho=$RM -> STOP"; echo "first core $RM $(date)" > STOP_128glm; exit 0; }
fi
LATEST=$(ls -t fc128glm/parthenon.out2.*.rhdf 2>/dev/null|head -1)
[ $N -ge 2 ] && [ -z "$LATEST" ] && { echo "no restart -> STOP"; echo "no-restart" > STOP_128glm; exit 0; }
...
mkdir -p fc128glm
echo "=== fc128 slot $N $(date) 128^3 curr_max_level=4 hydro/divergence_control=glm ===" >> fc128glm/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN $RA -t 07:45:00 \
  parthenon/mesh/nx1=128 parthenon/mesh/nx2=128 parthenon/mesh/nx3=128 \
  parthenon/mesh/numlevel=14 refinement/curr_max_level=4 hydro/divergence_control=glm parthenon/time/tlim=1.30 parthenon/time/nlim=100000000 \
