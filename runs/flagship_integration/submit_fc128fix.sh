...
N=$(cat fc128fix_n 2>/dev/null||echo 0); N=$((N+1)); echo $N > fc128fix_n
[ -f STOP_128fix ] && { echo "STOP -> exit"; exit 0; }
mkdir -p fc128fix
# first-core density check on the newest science dump
NEWEST=$(ls -t fc128fix/parthenon.out1.*.phdf 2>/dev/null|head -1)
if [ -n "$NEWEST" ]; then
  RM=$($PY - "$NEWEST" <<'PYEOF'
...
PYEOF
  ...
  [ "$($PY -c "print(1 if $RM*$RHO0>=$STOP_CGS else 0)")" = "1" ] && { echo "FIRST CORE rho=$RM -> STOP"; echo "first core $RM $(date)" > STOP_128fix; exit 0; }
fi
# restart: first slot from the external t=1.100 diagnostic restart; later from fc128fix's own
LATEST=$(ls -t fc128fix/parthenon.out2.*.rhdf 2>/dev/null|head -1)
...
echo "=== fc128fix slot $N $(date) binary=$(md5sum $BIN|cut -c1-8) restart=$RA ===" >> fc128fix/run.log
stdbuf -oL -eL mpirun -n 2 $MCA $WRAP $BIN $RA -t 07:45:00 \
  ...
  diffusion/ion_zeta=1.0e-16 -d fc128fix >> fc128fix/run.log 2>&1
echo "RUN_EXIT $? $(date)" >> fc128fix/run.log
