...
PY=/beegfs/u/bbg6470/venvs/analysis_env/bin/python; JOB=2414986
for i in $(seq 1 240); do
  st=$(squeue -j $JOB -h -o %T 2>/dev/null)
  reached=$(awk 'NR>2 && $1>=1.102905{print $1; exit}' probe_ctadoff/parthenon.out0.hst 2>/dev/null)
  if [ -z "$st" ] || [ -n "$reached" ]; then
    NEW=$(ls -t probe_ctadoff/parthenon.out1.*.phdf probe_ctadoff/parthenon.out2.*.rhdf 2>/dev/null | head -1)
...
PYEOF
    ...
    echo "VERDICT CT-AD-OFF: $R  (compare CT-AD-ON=0.689, GLM=0.107 @ t=1.10291). AD-off~0.689=>AD-under-CT BROKEN; AD-off>>0.689=>ideal-CT amplifies"
    exit 0
  fi
  sleep 20
done
