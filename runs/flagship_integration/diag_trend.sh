...
# Shared config: restart cycle 200 (t=1.100); advance 3 REAL cycles (nlim=240); print every cycle.
...
COMMON="parthenon/mesh/nx1=128 parthenon/mesh/nx2=128 parthenon/mesh/nx3=128 \
  parthenon/mesh/numlevel=14 refinement/curr_max_level=4 parthenon/time/tlim=100.0 parthenon/time/nlim=240 \
...
for CD in ct; do
  ...
  echo "=== DIAG2 ${CD^^}: restart fc128b/out2.00002 cyc200 t=1.100, nlim=240 (3 real cyc), binary=$(md5sum $BIN|cut -c1-8) $(date) ===" >> $OUT/run.log
